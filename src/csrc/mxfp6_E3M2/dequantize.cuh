...
__host__ void dequantize1d_host(TypeX const *x, const int M, TypeScale const *scales, const int group_size,
    ...
    for (int i = 0; i < M; ++i) {
        auto sign = static_cast<uint16_t>(hX[i] & 0x20) << 10;
        auto exp = static_cast<uint16_t>(hX[i] & 0x1C) >> 2;
        auto frac = static_cast<uint16_t>(hX[i] & 0x3) << 5;
...
__global__ static void dequantize1d_device(TypeX const *x, ShapeX shape_x, StrideX stride_x, // input
    ...
    for (int i = 0; i < size(tXrY); ++i) {
        ...
        auto sign = static_cast<uint16_t>(tXsX[i] & 0x20) << 10;
        auto exp = static_cast<uint16_t>(tXsX[i] & 0x1C) >> 2;
        auto frac = static_cast<uint16_t>(tXsX[i] & 0x3) << 5;
