...
from .dequantize import dequantize1d_E2M1, dequantize1d_E2M1_simulated
...
class PackedWeight:
    ...
    def unpack_accelerated(self) -> torch.Tensor:
        ...
        w = dequantize1d_E2M1(self.weight, self.scales, self.group_size).reshape(self.shape).to(self.dtype)
        return w
    ...
    def unpack_simulated(self) -> torch.Tensor:
        ...
        w = dequantize1d_E2M1_simulated(self.weight, self.scales, self.group_size).reshape(self.shape).to(self.dtype)
        return w
