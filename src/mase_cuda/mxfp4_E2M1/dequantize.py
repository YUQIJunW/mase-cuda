...
def dequantize1d_E2M1_reshape(input: torch.Tensor) -> torch.Tensor:
    assert input.ndim == 1, "Input tensor must be 1D"
    input = input.view(torch.uint8)
    high4 = input >> 4
    low4 = input & 0x0F
    output = torch.stack((high4, low4), dim=1).flatten()
    return output
...
def test_dequantize1d_E2M1_simulated():
    ...
    input_tensor = dequantize1d_E2M1_reshape(input_tensor)
