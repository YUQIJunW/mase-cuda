...
def dequantize1d_E3M2(input: torch.Tensor, scale: torch.Tensor, group_size: int) -> torch.Tensor:
    ...
    if max_grid_dim_y > max_num_ctas:
        ...
        for i in range(num_chunks):
            ...
            y_chunk = mase_cuda_ext.mxfp6_E3M2.dequantize1d(x_chunk, scale_chunk, group_size)
            chunks.append(y_chunk)
        output = torch.cat(chunks)
        output = output.reshape(ori_shape)
        input = input.reshape(ori_shape)
    else:
        output = mase_cuda_ext.mxfp6_E3M2.dequantize1d(input, scale, group_size)

    return output

def dequantize1d_E3M2_reshape(input: torch.Tensor) -> torch.Tensor:
...
def dequantize1d_E3M2_simulated(input: torch.Tensor, scale: torch.Tensor, group_size: int) -> torch.Tensor:
    ...
    exp = (fp6 & 0x1C).to(torch.int16) >> 2  # get the exponent bits
    frac = (fp6 & 0x03).to(torch.int16) << 5  # get the mantissa bits
...
def test_dequantize1d_E3M2_simulated():
    input_tensor = torch.tensor([193, 229,  90, 107, 209, 237], dtype=torch.uint8)
    scale_tensor = torch.tensor([122], dtype=torch.uint8)
    group_size = 8

    print(input_tensor.shape)
    input_tensor = dequantize1d_E3M2_reshape(input_tensor)
    output = dequantize1d_E3M2_simulated(input_tensor, scale_tensor, group_size)
...
if __name__ == "__main__":
    test_dequantize1d_E3M2_simulated()
