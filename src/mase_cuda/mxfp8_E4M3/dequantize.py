...
def dequantize1d_E4M3(input: torch.Tensor, scale: torch.Tensor, group_size: int) -> torch.Tensor:
    ...
    if max_grid_dim_y > max_num_ctas:
        ...
        for i in range(num_chunks):
            ...
            y_chunk = mase_cuda_ext.mxfp8_E4M3.dequantize1d(x_chunk, scale_chunk, group_size)
            chunks.append(y_chunk)
        output = torch.cat(chunks)
        output = output.reshape(ori_shape)
        input = input.reshape(ori_shape)
    else:
        output = mase_cuda_ext.mxfp8_E4M3.dequantize1d(input, scale, group_size)
